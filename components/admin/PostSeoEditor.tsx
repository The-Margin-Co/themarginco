...
import type { FormState, PostField } from "@/lib/validation";
import { PostSeoSection, type PostMeta } from "./cms/PostSeoSection";
import { Panel } from "./PostEditor";

type SeoAction = (state: FormState<PostField>, formData: FormData) => Promise<FormState<PostField>>;

/** SEO-only view of a post for SEO editors: the article itself is read-only. */
export function PostSeoEditor({
  action,
  post,
  initialMeta,
  initialSeo,
  ...
  action: SeoAction;
  post: { title: string; slug: string; excerpt: string; status: string; featuredImage: string };
  ...
  return (
    <form action={formAction} onSubmit={() => setSubmission({ meta, seo })} className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_20rem]">
      ...
      <aside className="space-y-6 xl:sticky xl:top-24 xl:self-start">
        ...
        >
          <p className="font-semibold text-fg">{post.title}</p>
          <p className="text-xs">Your role can change SEO settings only. Ask an admin to edit the article itself.</p>
          <button
            type="submit"
            disabled={pending}
