...
- [x] **M13 Production hardening**: security headers + CSP, signed `/og`, rate-limited `submit_lead()`, self-hosted videos in Supabase Storage (signed direct upload + server-side byte verification), production env guard, shared origin check, generic login error, graceful DB failure on list pages, `global-error.tsx`, carousel/focus a11y, apple icon + manifest, CI workflow, 46 unit tests (`npm test`). `tests/e2e/security.mjs` (RLS/API checks) still needs Docker to run.
- [~] **M13b Manual browser pass** (live site, signed in as admin). **Done:** blog write → publish; case study write → publish (appears on `/case-studies`, homepage and sitemap); video testimonial upload → publish; contact form → `/admin/leads` → mark handled → delete. **Still to do:** inline edit → save draft → publish → restore a revision; add an FAQ in the section panel; hide a section; replace an image; SEO editor flow; mobile layout (incl. carousel touch-swipe)
- [x] **M14 Deploy**: live at `https://themarginco.agency` (Vercel + GitHub `The-Margin-Co/themarginco`, cloud Supabase). All 9 migrations applied with `supabase db push` (CLI is logged in and linked; never `db reset`/seed). Public sign-ups are off (`auth/v1/settings` → `disable_signup: true`), the first admin exists, env vars are set
- [x] **M14b Live content + polish**: 3 honest "how we work" playbook case studies and 3 educational blog posts published with full SEO (no invented client results); 2 self-hosted testimonial videos published; rich-text editor fixed (it never rendered because `useEditorState` returns null until the first transaction); sitemap `revalidate` = 300 (`revalidatePath("/sitemap.xml")` did not purge it in production); admin Sign out moved to the top-right header; admin sidebar nav scrolls on its own; per-IP limit on the contact form (5/hour via salted IP hash, migration `20260919000000`); new **TMC monogram** logo + favicon (`components/layout/Logo.tsx`, `app/icon.svg`, `app/apple-icon.tsx` and the `/og` tile share the same paths)
- [ ] **M15 a11y/perf pass**: Lighthouse run
- [ ] **M16 Launch checklist**
  - [ ] Replace the placeholder **video testimonial names/roles** (the two published videos still need real client info) and the home/service hero metrics and text testimonials with verified data. Add real numbers + featured images to the playbook case studies if genuine client results exist. Case studies and videos are edited from `/admin/case-studies` and `/admin/video-testimonials`; the rest through the CMS or `lib/cms/defaults/*.ts`
  - [ ] Set the contact email under Admin → Site settings (default is `hello@yourdomain.com`)
  - [ ] Confirm the Calendly URL (edit under Admin → Site settings → Booking link; currently `calendly.com/d/dtkm-7wt-dxb/margin-co`) and the "$5k/month" minimum-budget FAQ answer
  - [x] `NEXT_PUBLIC_SITE_URL` set to the production domain
  - [x] Public sign-ups disabled in Supabase Auth
  - [x] Logo: TMC monogram replaces the generated "M" mark (swap in an official one only if the client supplies it)
  - [ ] Turn on MFA (TOTP) for staff and add Vercel Firewall rate limits for `/admin/login`
  - [ ] Add the site to Google Search Console and submit `/sitemap.xml` (also speeds up the favicon refresh in search results)
  - [ ] Run `tests/e2e/security.mjs` once Docker is available
